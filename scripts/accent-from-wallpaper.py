...
HYPR_OUT     = HOME / ".config/hypr/modules/colors.lua"
HYPR_OUT_OLD = HOME / ".config/hypr/modules/colors.conf"
...
def main() -> None:
    ...
    accent_rgba      = f"rgba({colors['primary'][1:]}ee)"
    accent_soft_rgba = f"rgba({colors['primaryContainer'][1:]}ee)"

    # Hyprland (Lua): required from modules/general.lua.
    HYPR_OUT.write_text(
        f"-- Generated by accent-from-wallpaper.py — do not edit by hand.\n"
        f"return {{\n"
        f'    accent     = "{accent_rgba}",\n'
        f'    accentSoft = "{accent_soft_rgba}",\n'
        f"}}\n"
    )

    # Hyprland (legacy .conf fallback): kept for users still on hyprland.conf.
    HYPR_OUT_OLD.write_text(
        f"# Generated by accent-from-wallpaper.py — do not edit by hand.\n"
        f"$accent     = {accent_rgba}\n"
        f"$accentSoft = {accent_soft_rgba}\n"
    )
