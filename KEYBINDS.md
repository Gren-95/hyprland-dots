...
| Key | Action |
|---|---|
| `Super+R` / `Alt+Space` | App launcher (Spotlight) — `Ctrl+D` hides the highlighted app, `←`/`→` flips to the hidden ones |
| `Super+Shift+D` | Hidden apps — pick one to show again (`↵`) |
...
| `Super+Shift+W` | Deal the previous card (wallpaper deck) |
